...
class Dataset:
    ...
    def next_valid_or_test_batch(self,batch_size,valid_or_test):
        if self.start_batch + batch_size > len(self.data[valid_or_test]):
            ret_facts = self.data[valid_or_test][self.start_batch : ]
            self.start_batch = 0
        else:
            ret_facts = self.data[valid_or_test][self.start_batch : self.start_batch + batch_size]
            self.start_batch += batch_size
        return shredFacts(ret_facts)
